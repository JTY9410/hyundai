...
"""
회원 필드 마이그레이션 스크립트
- member_type: '법인','개인' -> '법인사업자','개인사업자'
- 새로운 필드 추가: position, full_name, license_number, license_attachment_path
- 필수 필드 변경: 주소와 이메일을 제외한 필드들을 NOT NULL로 변경
"""
...
def migrate_member_fields():
    ...
    try:
        ...
        try:
            # 1. 기존 CHECK 제약조건 제거
            print("기존 CHECK 제약조건 제거 중...")
            cursor.execute("DROP INDEX IF EXISTS idx_member_business_number")
            cursor.execute("DROP INDEX IF EXISTS idx_member_created_at")
            cursor.execute("DROP INDEX IF EXISTS idx_member_partner_group")
            cursor.execute("DROP INDEX IF EXISTS idx_member_username_partner")

            # 제약조건이 있는 테이블을 재생성해야 하므로 임시 테이블 방식으로 진행
            print("기존 테이블 백업 및 재생성 준비 중...")

            # 2. 새로운 필드 추가를 위한 임시 테이블 생성
            ...
            # 3. 데이터 복사 및 member_type 업데이트
            print("데이터 복사 및 member_type 업데이트 중...")
            cursor.execute("""
            ...
            # 3. 새로운 제약조건 적용을 위한 임시 테이블 생성
            print("임시 테이블 생성 및 데이터 복사 중...")
            cursor.execute("""
            ...
            # 4. 기존 테이블을 새로운 구조의 테이블로 교체
            print("테이블 교체 중...")
            cursor.execute("DROP TABLE member")
            cursor.execute("ALTER TABLE member_temp RENAME TO member")

            # 5. 새로운 CHECK 제약조건 추가
            print("새로운 CHECK 제약조건 추가 중...")
            # SQLite에서는 ALTER TABLE로 CHECK 제약조건을 직접 추가할 수 없으므로
            # 새로운 테이블을 만들어서 다시 교체해야 함
            cursor.execute("""
